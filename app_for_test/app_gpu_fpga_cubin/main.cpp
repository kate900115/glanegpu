...
int main(int argc, char *argv[])
{
	...
	// Need to load module 
	char* module_file = (char*) "/home/zyuxuan/gpudma/app_gpu_fpga_cubin/cutemp/vadd.cubin";
	...
	uint64_t FPGAaddr = read_user_reg(0x1);
	...
	checkError(cuMemHostRegister(getAddrWithOffset(p,0x1), 20*sizeof(int), CU_MEMHOSTREGISTER_IOMEMORY));
	CUdeviceptr d_FPGAflag;
	checkError(cuMemHostGetDevicePointer(&d_FPGAflag, getAddrWithOffset(p,0x1),0));

	int* h_FPGAflag = (int*) getAddrWithOffset(p, 0x1);
		...
		for(unsigned i=0; i<state->page_count; i++) {
			...
			if (va == MAP_FAILED ) {
				...
				*GPUPointer = 1;				
				...
				checkError(cuLaunchKernel(function, 16, 16, 1, 16, 16, 1, 0, 0, args,0));
				...
				*value = 0;
				...
				// enable FPGA to write to the Lock on GPU thus release the lock 
				// and allow GPU to start execution
				// h_FPGAflag is an enable signal
				*h_FPGAflag = 1;
				cuCtxSynchronize();
